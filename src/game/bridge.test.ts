...
describe('LabBridge', () => {
  ...
  it('carries NPC proximity and dialogue state across the same boundary', () => {
    ...
    bridge.on('npc:activate', activate)

    bridge.emit('npc:nearby', { npcId: 'rook', anchor: { x: 720, y: 210 } })
    bridge.emit('npc:activate', { npcId: 'rook', anchor: { x: 720, y: 210 } })
    bridge.emit('ui:dialogue-change', { open: true, npcId: 'rook' })

    expect(nearby).toHaveBeenCalledWith({ npcId: 'rook', anchor: { x: 720, y: 210 } })
